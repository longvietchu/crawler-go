...
func main() {
	db, err := sql.Open("mysql", "longcv:123456@/coursera_crawler")
	if err != nil {
		panic(err)
	} else {
...
func insert(db *sql.DB, c Course) error {  
    query := "INSERT INTO courses(title, description, creator, level, url, language, commitment, rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    ...
    defer stmt.Close()
    res, err := stmt.ExecContext(ctx, c.Title, c.Description, c.Creator, c.Level, c.URL, c.Language, c.Commitment, c.Rating)
    if err != nil {
        log.Printf("Error %s when inserting row into courses table", err)
        return err
    }
    ...
    log.Printf("%d courses created ", rows)
    return nil
}